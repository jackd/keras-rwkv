...
class RwkvCausalLMPreprocessor(RwkvPreprocessor):  # pylint:disable=abstract-method
    """Rwkv Causal LM preprocessor.

    This preprocessing layer is meant for use with
    `keras_keras_rwkv.models.v4`. By default, it will take in batches of
    strings, and return outputs in a `(x, y, sample_weight)` format, where the
    `y` label is the next token id in the `x` sequence.

    For use with generation, the layer also exposes two methods
    `generate_preprocess()` and `generate_postprocess()`. When this preprocessor
    is attached to a `keras_rwkv.models.v4.RwkvCausalLM` instance, these methods
    will be called implicitly in `generate()`. They can also be called
    standalone (e.g. to precompute preprocessing inputs for generation in a
    separate process).

    Args:
        tokenizer: A `keras_rwkv.models.v4.RwkvTokenizer` instance.
    ...
    preprocessor = keras_rwkv.models.v4.RwkvCausalLMPreprocessor.from_preset(
        "rwkv-4-pile-169m"
    )
    ...
    def call(
        ...
        if y is not None or sample_weight is not None:
            logging.warning(
                "`RwkvCausalLMPreprocessor` generates `y` and `sample_weight` "
        ...
        x = self.tokenizer(x)  # pylint:disable=not-callable
