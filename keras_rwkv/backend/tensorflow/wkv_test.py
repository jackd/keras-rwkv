...
from keras_rwkv.backend.tensorflow import wkv as wkv_ops
...
def wkv_from_cuda(k, v, w, u):
    B, T, C = k.shape
    aa = np.zeros((B, C), dtype=k.dtype.name)
    bb = np.zeros((B, C), dtype=k.dtype.name)
    pp = np.full((B, C), -np.inf, dtype=k.dtype.name)
    output = np.zeros_like(v)
    for i in range(T):
        kk = k[:, i]
        vv = v[:, i]
...
class WkvTestCase(unittest.TestCase):
    def test_wkv_is_consistent(
        ...
        k, v, w, u = (tf.convert_to_tensor(x) for x in (k, v, w, u))
    ...
    def test_wkv_update_with_padding(
        ...
        next_vals, next_state = wkv_ops.wkv_update(
            tf.expand_dims(k[:, current_index + 1], 1),
            tf.expand_dims(v[:, current_index + 1], 1),
            w,
            u,
            block_state,
        )
        np.testing.assert_allclose(
            next_vals, tf.expand_dims(block_vals[:, current_index + 1], 1), rtol=1e-4
        )
